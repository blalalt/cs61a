...
def replace_leaf(t, old, new):
    ...
    "*** YOUR CODE HERE ***"
    def modify(sub_t):
        return new if is_leaf(sub_t) and label(sub_t)==old else label(sub_t)

    return tree(modify(t), [replace_leaf(sub_t, old, new) for sub_t in branches(t)])
...
def balanced(m): # 错误
    ...
    """
    "*** YOUR CODE HERE ***"
    # return total_weight(end(sides(m)[0])) == total_weight(end(sides(m)[1]))
    pass
...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        """Subtract amount from balance if funds are available."""
        fee: int = 0 if self.free_fee < FreeChecking.free_withdrawals \
                     else FreeChecking.withdraw_fee
        self.free_fee += 1
...
def make_withdraw(balance, password):
    ...
    def func(withdraw, p):
        nonlocal err_times, errors, balance
        if err_times >= 3:
            return "Your account is locked. Attempts: {}".format(errors)
        if p == password:
            if balance > withdraw:
...
def make_joint(withdraw, old_password, new_password):
    ...
    "*** YOUR CODE HERE ***"
    # from collections import defaultdict
    # w_p = defaultdict(lambda : []) # withdraw : [password]
    passwords: list = []
    # 验证原始密码
